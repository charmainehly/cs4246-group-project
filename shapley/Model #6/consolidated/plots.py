...
def create_parser():
    ...
    parser.add_argument(
        '--result_dir', default="rewards/exp_x1", type=str, help='Directory containing results of experiments')
    ...
    parser.add_argument("--plot-type", type=str, default="shapley_true",
                        help="type of diagram to plot: shapley_barchart/model_rewards/cat_plot")
    parser.add_argument("--model-dir", type=str, default="saves/run_x1/",
...
def plot_shap_barchart(shapley_values: List[float], agent_names: List[str]):
    ...
    print('shapley_values 3', shapley_values.iloc[2])
...
def plot_model_rewards_pp(folder_path: str, num_good_agents: int, num_preys: int):
    ...
    ax.set_title(f"Reward per episode on 20000 episodes")
...
if __name__ == "__main__":
    ...
    agent_names = [
        "Predator 0\n(slow)", "Predator 1\n(medium)", "Predator 2\n(fast)"]
    path_pp_mc = r"rewards/exp_x1"
    path_pp_mc_true = r"rewards/true-shap-exp_x1"
    ...
    if args.plot_type == "shapley_barchart":
        ...
        for each in range(len(data[0])):
            ...
            if (data[2][each] == "replace (real)"):
                if (data[0][each] == 0):
                    true_replace_0 += data[1][each]
                if (data[0][each] == 1):
                    true_replace_1 += data[1][each]
                if (data[0][each] == 2):
                    true_replace_2 += data[1][each]

            if (data[2][each] == "replace (MC estimation)"):
                if (data[0][each] == 0):
                    mc_replace_0 += data[1][each]
                if (data[0][each] == 1):
                    mc_replace_1 += data[1][each]
                if (data[0][each] == 2):
                    mc_replace_2 += data[1][each]

            if (data[2][each] == "random (real)"):
                if (data[0][each] == 0):
                    true_random_0 += data[1][each]
                if (data[0][each] == 1):
                    true_random_1 += data[1][each]
                if (data[0][each] == 2):
                    true_random_2 += data[1][each]

            if (data[2][each] == "random (MC estimation)"):
                if (data[0][each] == 0):
                    mc_random_0 += data[1][each]
                if (data[0][each] == 1):
                    mc_random_1 += data[1][each]
                if (data[0][each] == 2):
                    mc_random_2 += data[1][each]

            if (data[2][each] == "noop (real)"):
                if (data[0][each] == 0):
                    true_noop_0 += data[1][each]
                if (data[0][each] == 1):
                    true_noop_1 += data[1][each]
                if (data[0][each] == 2):
                    true_noop_2 += data[1][each]

            if (data[2][each] == "noop (MC estimation)"):
                if (data[0][each] == 0):
                    mc_noop_0 += data[1][each]
                if (data[0][each] == 1):
                    mc_noop_1 += data[1][each]
                if (data[0][each] == 2):
                    mc_noop_2 += data[1][each]

                
        mc_replace_0 = mc_replace_0/3
        mc_replace_1 = mc_replace_1/3
        mc_replace_2 = mc_replace_2/3

        mc_random_0 = mc_random_0/3
        mc_random_1 = mc_random_1/3
        mc_random_2 = mc_random_2/3


        mc_noop_0 = mc_noop_0/3
        mc_noop_1 = mc_noop_1/3
        mc_noop_2 = mc_noop_2/3


        true_replace_0 = true_replace_0/3
        true_replace_1 = true_replace_1/3
        true_replace_2 = true_replace_2/3

        true_random_0 = true_random_0/3
        true_random_1 = true_random_1/3
        true_random_2 = true_random_2/3


        true_noop_0 = true_noop_0/3
        true_noop_1 = true_noop_1/3
        true_noop_2 = true_noop_2/3
        ...
        print(true_replace_0)
        print(true_replace_1)
        print(true_replace_2)
        print(true_random_0)
        print(true_random_1)
        print(true_random_2)
        print(true_noop_0)
        ...
        plot_shapley_vs_speed("rewards/exp_x1", 0)
    elif args.plot_type == "goal_agents":
        plot_goal_agents_pp_one(r"goal_agents/exp_x1", agent_names)
        plot_goal_agents_pp_one(r"goal_agents/exp_x1", agent_names)
