            ...
            for behaviour in missing_agents_behaviours:
                for speeds in agent_speeds:
                    ...
                    if not os.path.exists(fname):
                        command = f'python run.py --load-dir "saves/{run}/episode_1000/model" --missing-agents-behaviour {behaviour} --exp-name {exp_name} --save-dir {folder_name} --shapley-M {M} --num-episodes 1 --agent-speeds'
                        for speed in speeds:
                            command += f" {speed}"
        ...
        for run in runs:
            for behaviour in missing_agents_behaviours:
                for speeds in agent_speeds:
                    ...
                    if not os.path.exists(fname):
                        print(fname)
                        command = f'python run.py --load-dir "saves/{run}/episode_1000/model" --missing-agents-behaviour {behaviour} --exp-name {run}_{behaviour}_{n} --true-shapley --save-dir {folder_name} --num-episodes {n_episodes} --agent-speeds'
                        for speed in speeds:
                            command += f" {speed}"
...
def save_goal_agents(N, folder_name, agent_speeds):
    ...
    for run in runs:
            ...
            if not os.path.exists(fname):
                command = f'python run.py --load-dir "saves/{run}/episode_1000/model" --exp-name {exp_name} --save-dir {folder_name} --rollout --num-episodes {N} --agent-speeds'
...
if __name__ == "__main__":
    ...
    save_goal_agents(100, "goal_agents/exp_x1",
                      agent_speeds=SPEEDS_EXP12)
    start_time = time.time()    
    save_rewards_true_shapley(N=1, n_episodes=1000, folder_name="rewards/true-shap-exp_x1",
                               agent_speeds=SPEEDS_EXP1)
